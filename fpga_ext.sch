EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 10 14
Title "Overengineered Expansion Connector"
Date ""
Rev ""
Comp "Oxide Computer"
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Connector_Generic:Conn_02x24_Odd_Even J1001
U 1 1 5F327423
P 6100 3350
F 0 "J1001" H 6150 4667 50  0000 C CNN
F 1 "EXPANSION" H 6150 4576 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_2x24_P2.54mm_Vertical" H 6100 3350 50  0001 C CNN
F 3 "~" H 6100 3350 50  0001 C CNN
	1    6100 3350
	1    0    0    -1  
$EndComp
Text HLabel 6400 3350 2    50   Input ~ 0
CLK
Text HLabel 5900 3150 0    50   Input ~ 0
~WE
Text HLabel 5900 3250 0    50   Input ~ 0
~LE
Text HLabel 6400 3150 2    50   Input ~ 0
~OE
Text HLabel 6400 3050 2    50   Input ~ 0
~CE
Text HLabel 5900 2950 0    50   Input ~ 0
~BL0
Text HLabel 5900 2850 0    50   Input ~ 0
~BL1
$Comp
L power:GND #PWR01008
U 1 1 5F33063B
P 5900 4550
F 0 "#PWR01008" H 5900 4300 50  0001 C CNN
F 1 "GND" V 5905 4422 50  0000 R CNN
F 2 "" H 5900 4550 50  0001 C CNN
F 3 "" H 5900 4550 50  0001 C CNN
	1    5900 4550
	0    1    1    0   
$EndComp
$Comp
L power:+3V3 #PWR01007
U 1 1 5F3308F6
P 5900 4250
F 0 "#PWR01007" H 5900 4100 50  0001 C CNN
F 1 "+3V3" V 5915 4378 50  0000 L CNN
F 2 "" H 5900 4250 50  0001 C CNN
F 3 "" H 5900 4250 50  0001 C CNN
	1    5900 4250
	0    -1   -1   0   
$EndComp
$Comp
L power:+3V3 #PWR01005
U 1 1 5F330AA5
P 5900 3650
F 0 "#PWR01005" H 5900 3500 50  0001 C CNN
F 1 "+3V3" V 5915 3778 50  0000 L CNN
F 2 "" H 5900 3650 50  0001 C CNN
F 3 "" H 5900 3650 50  0001 C CNN
	1    5900 3650
	0    -1   -1   0   
$EndComp
$Comp
L power:+3V3 #PWR01003
U 1 1 5F3310BE
P 5900 3050
F 0 "#PWR01003" H 5900 2900 50  0001 C CNN
F 1 "+3V3" V 5915 3178 50  0000 L CNN
F 2 "" H 5900 3050 50  0001 C CNN
F 3 "" H 5900 3050 50  0001 C CNN
	1    5900 3050
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR01006
U 1 1 5F331418
P 5900 3950
F 0 "#PWR01006" H 5900 3700 50  0001 C CNN
F 1 "GND" V 5905 3822 50  0000 R CNN
F 2 "" H 5900 3950 50  0001 C CNN
F 3 "" H 5900 3950 50  0001 C CNN
	1    5900 3950
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR01004
U 1 1 5F3317E3
P 5900 3350
F 0 "#PWR01004" H 5900 3100 50  0001 C CNN
F 1 "GND" V 5905 3222 50  0000 R CNN
F 2 "" H 5900 3350 50  0001 C CNN
F 3 "" H 5900 3350 50  0001 C CNN
	1    5900 3350
	0    1    1    0   
$EndComp
$Comp
L power:+3V3 #PWR01016
U 1 1 5F331C70
P 6400 4350
F 0 "#PWR01016" H 6400 4200 50  0001 C CNN
F 1 "+3V3" V 6415 4478 50  0000 L CNN
F 2 "" H 6400 4350 50  0001 C CNN
F 3 "" H 6400 4350 50  0001 C CNN
	1    6400 4350
	0    1    1    0   
$EndComp
$Comp
L power:+3V3 #PWR01014
U 1 1 5F3323FB
P 6400 3750
F 0 "#PWR01014" H 6400 3600 50  0001 C CNN
F 1 "+3V3" V 6415 3878 50  0000 L CNN
F 2 "" H 6400 3750 50  0001 C CNN
F 3 "" H 6400 3750 50  0001 C CNN
	1    6400 3750
	0    1    1    0   
$EndComp
$Comp
L power:+3V3 #PWR01012
U 1 1 5F332974
P 6400 3250
F 0 "#PWR01012" H 6400 3100 50  0001 C CNN
F 1 "+3V3" V 6415 3378 50  0000 L CNN
F 2 "" H 6400 3250 50  0001 C CNN
F 3 "" H 6400 3250 50  0001 C CNN
	1    6400 3250
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR01015
U 1 1 5F333841
P 6400 4050
F 0 "#PWR01015" H 6400 3800 50  0001 C CNN
F 1 "GND" V 6405 3922 50  0000 R CNN
F 2 "" H 6400 4050 50  0001 C CNN
F 3 "" H 6400 4050 50  0001 C CNN
	1    6400 4050
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR01013
U 1 1 5F333E54
P 6400 3450
F 0 "#PWR01013" H 6400 3200 50  0001 C CNN
F 1 "GND" V 6405 3322 50  0000 R CNN
F 2 "" H 6400 3450 50  0001 C CNN
F 3 "" H 6400 3450 50  0001 C CNN
	1    6400 3450
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR01011
U 1 1 5F334346
P 6400 2950
F 0 "#PWR01011" H 6400 2700 50  0001 C CNN
F 1 "GND" V 6405 2822 50  0000 R CNN
F 2 "" H 6400 2950 50  0001 C CNN
F 3 "" H 6400 2950 50  0001 C CNN
	1    6400 2950
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR01002
U 1 1 5F335D9F
P 5900 2750
F 0 "#PWR01002" H 5900 2500 50  0001 C CNN
F 1 "GND" V 5905 2622 50  0000 R CNN
F 2 "" H 5900 2750 50  0001 C CNN
F 3 "" H 5900 2750 50  0001 C CNN
	1    5900 2750
	0    1    1    0   
$EndComp
Text HLabel 6400 2850 2    50   Output ~ 0
~IRQ
Text HLabel 5900 2650 0    50   Input ~ 0
SPI_CLK
Text HLabel 6400 2750 2    50   Output ~ 0
SPI_MISO
$Comp
L power:GND #PWR01009
U 1 1 5F357D29
P 6400 2350
F 0 "#PWR01009" H 6400 2100 50  0001 C CNN
F 1 "GND" V 6405 2222 50  0000 R CNN
F 2 "" H 6400 2350 50  0001 C CNN
F 3 "" H 6400 2350 50  0001 C CNN
	1    6400 2350
	0    -1   -1   0   
$EndComp
Text HLabel 6400 2550 2    50   Input ~ 0
SPI_MOSI
$Comp
L power:+3V3 #PWR01010
U 1 1 5F3588DB
P 6400 2650
F 0 "#PWR01010" H 6400 2500 50  0001 C CNN
F 1 "+3V3" V 6415 2778 50  0000 L CNN
F 2 "" H 6400 2650 50  0001 C CNN
F 3 "" H 6400 2650 50  0001 C CNN
	1    6400 2650
	0    1    1    0   
$EndComp
Text HLabel 5900 2550 0    50   Input ~ 0
~SPI_CS0
Text HLabel 6400 2250 2    50   Input ~ 0
SCL
Text HLabel 5900 2350 0    50   BiDi ~ 0
SDA
Text HLabel 6400 2450 2    50   Input ~ 0
~SPI_CS1
Text Label 6400 4550 0    50   ~ 0
DA0
Text Label 6400 4450 0    50   ~ 0
DA2
Text Label 6400 4250 0    50   ~ 0
DA4
Text Label 6400 4150 0    50   ~ 0
DA6
Text Label 6400 3950 0    50   ~ 0
DA8
Text Label 6400 3850 0    50   ~ 0
DA10
Text Label 6400 3650 0    50   ~ 0
DA12
Text Label 6400 3550 0    50   ~ 0
DA14
Text Label 5900 4450 2    50   ~ 0
DA1
Text Label 5900 4350 2    50   ~ 0
DA3
Text Label 5900 4150 2    50   ~ 0
DA5
Text Label 5900 4050 2    50   ~ 0
DA7
Text Label 5900 3850 2    50   ~ 0
DA9
Text Label 5900 3750 2    50   ~ 0
DA11
Text Label 5900 3550 2    50   ~ 0
DA13
Text Label 5900 3450 2    50   ~ 0
DA15
Text Label 7700 2700 2    50   ~ 0
DA0
Text Label 7700 2800 2    50   ~ 0
DA1
Text Label 7700 2900 2    50   ~ 0
DA2
Text Label 7700 3000 2    50   ~ 0
DA3
Text Label 7700 3100 2    50   ~ 0
DA4
Text Label 7700 3200 2    50   ~ 0
DA5
Text Label 7700 3300 2    50   ~ 0
DA6
Text Label 7700 3400 2    50   ~ 0
DA7
Text Label 7700 3500 2    50   ~ 0
DA8
Text Label 7700 3600 2    50   ~ 0
DA9
Text Label 7700 3700 2    50   ~ 0
DA10
Text Label 7700 3800 2    50   ~ 0
DA11
Text Label 7700 3900 2    50   ~ 0
DA12
Text Label 7700 4000 2    50   ~ 0
DA13
Text Label 7700 4100 2    50   ~ 0
DA14
Text Label 7700 4200 2    50   ~ 0
DA15
Wire Wire Line
	7700 2700 7750 2700
Wire Wire Line
	7700 2800 7750 2800
Wire Wire Line
	7700 2900 7750 2900
Wire Wire Line
	7700 3000 7750 3000
Wire Wire Line
	7700 3100 7750 3100
Wire Wire Line
	7700 3200 7750 3200
Wire Wire Line
	7700 3300 7750 3300
Wire Wire Line
	7700 3400 7750 3400
Wire Wire Line
	7700 3500 7750 3500
Wire Wire Line
	7700 3600 7750 3600
Wire Wire Line
	7700 3700 7750 3700
Wire Wire Line
	7700 3800 7750 3800
Wire Wire Line
	7700 3900 7750 3900
Wire Wire Line
	7700 4000 7750 4000
Wire Wire Line
	7700 4100 7750 4100
Wire Wire Line
	7700 4200 7750 4200
Entry Wire Line
	7750 2700 7850 2800
Entry Wire Line
	7750 2800 7850 2900
Entry Wire Line
	7750 2900 7850 3000
Entry Wire Line
	7750 3000 7850 3100
Entry Wire Line
	7750 3100 7850 3200
Entry Wire Line
	7750 3200 7850 3300
Entry Wire Line
	7750 3300 7850 3400
Entry Wire Line
	7750 3400 7850 3500
Entry Wire Line
	7750 3500 7850 3600
Entry Wire Line
	7750 3600 7850 3700
Entry Wire Line
	7750 3700 7850 3800
Entry Wire Line
	7750 3800 7850 3900
Entry Wire Line
	7750 3900 7850 4000
Entry Wire Line
	7750 4000 7850 4100
Entry Wire Line
	7750 4100 7850 4200
Entry Wire Line
	7750 4200 7850 4300
Text Label 7900 4300 0    50   ~ 0
DA[0..15]
Wire Bus Line
	7850 4300 8300 4300
Text HLabel 8300 4300 2    50   BiDi ~ 0
DA[0..15]
Text HLabel 5900 2250 0    50   Input ~ 0
~RESET
$Comp
L power:+12V #PWR01001
U 1 1 5F3C0A1E
P 5900 2450
F 0 "#PWR01001" H 5900 2300 50  0001 C CNN
F 1 "+12V" V 5915 2578 50  0000 L CNN
F 2 "" H 5900 2450 50  0001 C CNN
F 3 "" H 5900 2450 50  0001 C CNN
	1    5900 2450
	0    -1   -1   0   
$EndComp
Text Notes 4950 5150 0    50   ~ 0
Notes:\n- Return paths near all fast signals (which is nearly everything).\n- 12V available in case we need 5V or something\n- A/D muxed because demuxing in FPGA is free\n- Two SPI selects to address (e.g.) both FPGA and bitstream ROM\n- I2C because why not\n- Only one CE routed (of 2 avail) because what is CE1 for anyway
Wire Bus Line
	7850 2800 7850 4300
$EndSCHEMATC
